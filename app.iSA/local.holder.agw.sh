...
#.est
__version__="""0.00xz"""
__author__="""[<iSA>!#a MVC pattern selection-^MENU^>${__version__}<]"""
#
COLUMNS=1
#
ui_scr_maxcols=80
...
ui_label_holder='-\033[94m%s\033[0m:-\033[93m%s\033[0m'
...
function do_init()
{
    ...
    function isa-set--delegate-on () { export __pragma__set_delegate=true ; }
    function isa-set--delegate-off() { export __pragma__set_delegate=false; }
...
function do_args()
{
    ...
    if [[ "${#__static__argv[@]}" == 0 ]];
    then echo -n
    else __static__vptr="${__static__argv[0]}.sh"
    fi
}
#
function do_load()
{
    ...
    then
        ...
        printf -- "%s%s\n" "${string_display}" "${ui_span_hr2tail1:6+3+${#__author__}}"
        local string_display="$(printf -- "${ui_label_holder}" "holder" "${__static__dptr}")"
        printf -- "%s%s\n" "${string_display}" "${ui_span_hr2tail1:6+3+${#__static__dptr}}"
    fi
    #
    for slice in "${array_cptr[@]}";
    do
        local string_dptr+=".${slice}"
        string_dptr="${string_dptr##.}"
        local realpath_mptr="$(self_dirname)/${string_dptr}.model.sh"
        ...
        then
            ...
            printf -- "%s%s\n" "${string_display}" "${ui_span_hr2tail1:6+3+${#string_mptr}}"
        fi
    done
    #
    if [[ "${__pragma__set_verbose}" == true ]];
...
function do_call()
{
    ...
    if [[ ! "${#__static__argv[@]}" == 0 ]];
    then $(echo "${__static__vptr}" | grep -q "${__static__dptr}") || return 0
    fi
    if [[ ! "${#__static__argv[@]}" == 0 ]];
    then [[ ! -f "$(self_dirname)/${__static__vptr}" ]] && return 0
    fi
    #
    if [[ "${__pragma__set_verbose}" == true ]];
    then
        local string_called="${__static__vptr}"
        ...
        printf -- "%s%s\n" "${string_display}" "${ui_span_hr2tail1:6+3+${#string_vptr}}"
        printf "%s\n" "${ui_span_hr4tail1}"
        printf "%s\n" "${ui_span_hr5tail1}"
    fi
    #
    local timestart="$(date +"%s")"
    source "$(self_dirname)/${__static__vptr}"
